...
from app.ai_provider import request_chat_completion, to_data_image_url
...
15. has_image=true 时，question_image_bbox 必须给出“只包含这道题中非文字图形/表格/示意图信息”的裁剪框，不要包含整张卷子，也不要包含上一题、下一题、无关草稿、批改痕迹或大量题干文字。
16. 如果一道选择题的 A/B/C/D 选项本身是图形、表格或涂色区域，question_image_bbox 必须覆盖这道题的所有相关图形选项，不能只框其中一个选项，也不能截断任何选项底部或右侧。
17. question_image_bbox 使用归一化坐标 [x1, y1, x2, y2]，取值 0 到 1，分别表示相对整张图片宽高的位置。坐标要略微留白，确保图形完整；上边界尽量贴近本题图形，不要扩到上一题。
18. 如果题目是纯文字题，has_image 为 false，question_image_bbox 为 null。
19. 对于几何图题，question_text 只写题目文字要求；图形本身通过 question_image_bbox 保存。
        ...
        [
            {
                ...
                "content": [
                    ...
                    {
                        "type": "image_url",
                        "image_url": {"url": to_data_image_url(file.content_type, image_bytes)},
                    },
                ],
            }
        ],
